...
# usage: chessboard_finder.py [-h] images [images ...]

# Find orthorectified chessboard corners in image

# positional arguments:
#   images      Input image URLs or local file paths
...
def main(input_path):
  if input_path.startswith('http'):
    print("Loading url %s..." % input_path)
    color_img, url = loadImageFromURL(input_path)
    source = url
  else:
    print("Loading local image %s..." % input_path)
    try:
      color_img = loadImageFromPath(input_path)
      source = input_path
    except Exception as e:
      print('Couldn\'t load image file: %s - %s' % (input_path, str(e)))
      return
  
  # Fail if can't load image
  if color_img is None:
    print('Couldn\'t load: %s' % source)
    return
  ...
  if corners is not None:
    print("\tFound corners for %s: %s" % (source, corners))
    if input_path.startswith('http'):
      link = getVisualizeLink(corners, source)
      print(link)
...
if __name__ == '__main__':
  ...
  parser.add_argument('images', default=['input.png'],
    metavar='images', type=str,  nargs='*', help='Input image URLs or local file paths')
